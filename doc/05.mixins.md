...
```fn
@include gridle_parent_vertical_align( );
@include gridle_parent_vertical_align( {state} );
```
...
#sidebar {
	...
	/**
	 * You can specify multiple states directly width gridle_stateS :
...
> To target multiple registered states in one time, you can use the gridle_state**s** mixin like this : **@include gridle_states( mobile tablet );**
